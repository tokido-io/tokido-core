 ...
 * <ul>
 *   <li>{@code auth_time} — emitted when {@code AuthorizationCodeData.authTime}
 *       is non-null (RC1 always emits when available; per OIDC Core, required
 *       only for {@code prompt=login}/{@code max_age} or when the client has
 *       {@code requireAuthTime} set)</li>
 *   <li>{@code nonce} — echoed when present in the captured code data</li>
...
final class IdTokenBuilder {
     ...
     * @param codeData      captured at /authorize; non-null
    ...
    String build(String subjectId,
        ...
        Objects.requireNonNull(codeData, "codeData");
